...
class GitPusher(threading.Thread):
    ...
    def _do_push(self):
        with self.lock:
            try:
                subprocess.run(
                    [self.git, "add", "--force", self.dynamic_rel],
                ...
                for attempt in range(4):
                    # Pull --rebase before push to handle remote commits
                    # (e.g. github-actions board sync commits on main)
                    if self.branch:
                        subprocess.run(
...
def main():
    ...
    parser.add_argument("--cleanup",       action="store_true",       help="Delete all clips on exit (discard mode)")
    ...
    # Clean up residual clips from previous session (files + git tracking)
    cleaned = 0
    stale_files = []
    for f in os.listdir(DYNAMIC_DIR):
        if f.startswith("clip_") and f.endswith(".mp4"):
            fpath = os.path.join(DYNAMIC_DIR, f)
            os.remove(fpath)
            stale_files.append(os.path.join("live/dynamic", f))
            cleaned += 1
    if cleaned:
        print(f"[LIVE] Cleaned {cleaned} residual clips from previous session")
        # Also remove from git index if tracked (prevents restart conflicts)
        git = _git_cmd()
        for rel in stale_files:
            subprocess.run(
                [git, "rm", "--cached", "--ignore-unmatch", "-q", rel],
                cwd=REPO_ROOT, capture_output=True, timeout=5
            )
        # Commit the cleanup so push thread starts clean
        subprocess.run(
            [git, "add", "live/dynamic"],
            cwd=REPO_ROOT, capture_output=True, timeout=5
        )
        result = subprocess.run(
            [git, "commit", "-m", "live: cleanup stale clips from previous session"],
            cwd=REPO_ROOT, capture_output=True, text=True, timeout=10
        )
        if result.returncode == 0:
            print(f"[LIVE] Git: committed cleanup of {cleaned} stale clips")
    ...
    while running:
        ...
        try:
            ...
            if len(frame_buffer) >= clip_frames:
                ...
                # Delete existing file before encoding to avoid overwrite conflicts
                # (git push thread may hold a read handle on the old file)
                if os.path.exists(clip_path):
                    try:
                        os.remove(clip_path)
                    except OSError as e:
                        print(f"  [LIVE] WARN: could not remove old {os.path.basename(clip_path)}: {e}")
            ...
            if "RTSP read failed" in str(e) and args.rtsp:
                print(f"[LIVE] RTSP stream dropped — reconnecting...")
                frame_buffer.clear()
                try:
                    cap.release()
                    time.sleep(2)
                    import cv2
                    cap = cv2.VideoCapture(fix_wsl_url(args.rtsp))
                    if cap.isOpened():
                        print(f"[LIVE] RTSP reconnected")
                    else:
                        print(f"[LIVE] RTSP reconnect failed — retrying in 5s...")
                        time.sleep(5)
                        cap = cv2.VideoCapture(fix_wsl_url(args.rtsp))
                except Exception as re:
                    print(f"[LIVE] RTSP reconnect error: {re}")
                    time.sleep(5)
                continue
    ...
    if args.cleanup:
        print("[LIVE] Cleanup mode — discarding all clips...")
        try:
            from clip_discard import discard_all, print_summary
            summary = discard_all(local_only=args.no_push)
            print_summary(summary)
        except ImportError:
            # Fallback: just delete local clips
            cleaned = 0
            for f in os.listdir(DYNAMIC_DIR):
                if f.endswith(".mp4") and not f.startswith("."):
                    ...
                    cleaned += 1
            if cleaned:
                print(f"[LIVE] Fallback cleanup: deleted {cleaned} clips")
